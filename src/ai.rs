...
impl AISummarizerImpl {
    ...
    // For testing purposes - allows injecting a custom client
    #[cfg(test)]
    fn with_client(client: Client<OpenAIConfig>, model: String, max_tokens: u32) -> Self {
        Self {
            client,
            model,
            max_tokens,
...
mod tests {
    ...
    async fn test_summarize_commits_empty() {
        let summarizer = AISummarizerImpl::new("gpt-4o".to_string(), 1000);
    ...
    async fn test_summarize_commits_error() {
        ...
        let summarizer = AISummarizerImpl::new("invalid-model".to_string(), 1000);
